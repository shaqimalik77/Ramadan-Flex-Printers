  ...
  return (
    <div>
        ...
        <ul className="mt-6 space-y-2.5">
          {product.features.map((feature) => (
            <li key={feature} className="flex items-start gap-2.5 text-sm text-neutral-700 sm:text-[15px]">
              <span className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#E41F26]" />
              {feature}
            </li>
          ))}
        </ul>
      ...
      <Link
        ...
        className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl bg-[#E41F26] py-4 text-base font-semibold text-white transition-colors duration-200 hover:bg-[#b91219] sm:w-auto sm:px-12 shadow-sm"
      >
